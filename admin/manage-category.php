...
if(isset($_POST['add_category'])) {
    ...
    $sql = "INSERT INTO tblcategory (CategoryName, CreationDate) VALUES (:name, NOW())";
    $query = $dbh->prepare($sql);
...
<head>
<meta charset="UTF-8">
<title>Bus Pass Management System | Manage Bus Category</title>
...
<div class="main-content">
    <div class="header">
        <h2>Manage Bus Categories</h2>
        <a href="logout.php" class="logout"><i class="fa fa-sign-out-alt"></i> Logout</a>
    </div>

    <!-- Add Category Button -->
    <button class="add-category-btn" id="openModal"><i class="fa fa-plus"></i> Add New Bus Category</button>
                ...
                if($query->rowCount() > 0) {
                    foreach($results as $row) { ?>
                        <tr>
                            ...
                            <td>
                                <a class="btn-action btn-edit" href="edit-category-detail.php?editid=<?php echo htmlentities($row->ID); ?>"><i class="fa fa-edit"></i> Edit</a>
                                <a class="btn-action btn-delete" href="manage-category.php?delid=<?php echo htmlentities($row->ID); ?>" onclick="return confirm('Do you really want to delete?');"><i class="fa fa-trash"></i> Delete</a>
                            </td>
                        </tr>
                    <?php $cnt++; } } ?>
            </tbody>
...
<div id="categoryModal" class="modal">
    <div class="modal-content">
        ...
        <h3>Add New Bus Category</h3>
        <form method="post">
            <div style="margin-bottom:15px;">
                <input type="text" name="categoryname" placeholder="Category Name" required style="width:100%; padding:10px; border-radius:6px; border:1px solid #ccc;">
            </div>
            <button type="submit" name="add_category" style="background:#1e90ff; color:#fff; padding:10px 20px; border:none; border-radius:6px; cursor:pointer;">Add Category</button>
        </form>
    </div>
</div>
